...
import cn.td.aiot.decoder.utils.commandManager.commandbuidler.CommandAssembly;
import cn.td.aiot.decoder.utils.commandManager.commandbuidler.CommandAssemblyImpl;
import cn.td.aiot.decoder.utils.commandManager.commandbuidler.CommandBuidler;
...
import cn.td.aiot.decoder.utils.commandManager.data.TaskDaoImpl;
import cn.td.aiot.decoder.utils.commandManager.handler.DefaultOutHandlerMethod;
import cn.td.aiot.decoder.utils.commandManager.handler.KeepAliveHandler;
import cn.td.aiot.decoder.utils.commandManager.handler.OutHandlerMethod;
import cn.td.aiot.decoder.utils.commandManager.handler.TaskHandler;
import cn.td.aiot.decoder.utils.commandManager.handler.TaskHandlerImpl;
...
public class CommandManagerImpl implements CommandManager {
	...
	public void init(Integer size) {
		...
		boolean iskeepalive=config.isKeepalive();;
		if (size == null) {
...
//			iskeepalive=config.isKeepalive();
	...
	public String start(String id, String command, boolean hasPath) {
		...
		if (id != null && command != null) {
			...
			System.err.println(hasPath ? command : config.getPath() + command);

			tasker = taskHandler.process(id, hasPath ? command : config.getPath() + command);
	...
	public boolean stop(String id) {
		if (id != null && taskDao.isHave(id)) {
			if (config.isDebug()) {
				System.err.println("任务：" + query(id).toString());
